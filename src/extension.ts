...
import { createDalvikAPI } from "./exports"
...
export async function activate(context: vscode.ExtensionContext) {
	outputChannel = vscode.window.createOutputChannel("Dalvik")
	context.subscriptions.push(outputChannel)

	ErrorService.initialize()
	Logger.initialize(outputChannel)
	Logger.log("Dalvik extension activated")

	maybeSetupHostProviders(context)

	// Migrate custom instructions to global Dalvik rules (one-time cleanup)
	...
	const previousVersion = context.globalState.get<string>("dalvikVersion")
	...
	vscode.commands.executeCommand("setContext", "dalvik.isDevMode", IS_DEV && IS_DEV === "true")
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`Dalvik version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			const lastShownPopupNotificationVersion = context.globalState.get<string>("dalvikLastPopupNotificationVersion")
			...
			if (currentVersion !== lastShownPopupNotificationVersion && previousVersion) {
				...
				const message = `Dalvik has been updated to v${currentVersion}`
				...
				await context.globalState.update("dalvikLastPopupNotificationVersion", currentVersion)
			}
			// Always update the main version tracker for the next launch.
			await context.globalState.update("dalvikVersion", currentVersion)
		}
	} catch (error) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.mcpButtonClicked", (webview: any) => {
	...
	const openDalvikInNewTab = async () => {
		Logger.log("Opening Dalvik in new tab")
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "Dalvik", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(vscode.commands.registerCommand("dalvik.popoutButtonClicked", openDalvikInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("dalvik.openInNewTab", openDalvikInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.settingsButtonClicked", (webview: any) => {
			const isSidebar = !webview
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.historyButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.accountButtonClicked", (webview: any) => {
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				...
				Logger.log("Dalvik dev task commands registered")
			})
			.catch((error) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			await vscode.commands.executeCommand("dalvik.focusChatInput") // Ensure Dalvik is visible and input focused
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Dalvik (Always available)
					const addAction = new vscode.CodeAction("Add to Dalvik", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: "dalvik.addToChat",
						title: "Add to Dalvik",
						arguments: [expandedRange, context.diagnostics],
					}
					actions.push(addAction)

					// Explain with Dalvik (Always available)
					const explainAction = new vscode.CodeAction("Explain with Dalvik", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: "dalvik.explainCode",
						title: "Explain with Dalvik",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Dalvik (Always available)
					const improveAction = new vscode.CodeAction("Improve with Dalvik", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: "dalvik.improveCode",
						title: "Improve with Dalvik",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Dalvik (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Dalvik", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: "dalvik.fixWithDalvik",
							title: "Fix with Dalvik",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.fixWithDalvik", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			// Add this line to focus the chat input first
			await vscode.commands.executeCommand("dalvik.focusChatInput")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.explainCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("dalvik.focusChatInput") // Ensure Cline is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
				return
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.improveCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("dalvik.focusChatInput") // Ensure Cline is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.focusChatInput", async () => {
			...
			if (activeWebviewProvider?.getWebview() && activeWebviewProvider.getWebview().hasOwnProperty("reveal")) {
				...
				if (!activeWebviewProvider) {
					...
					await vscode.commands.executeCommand("dalvik.openInNewTab")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.openWalkthrough", async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("dalvik.generateGitCommitMessage", async () => {
	...
	return createDalvikAPI(outputChannel, sidebarWebview.controller)
}
